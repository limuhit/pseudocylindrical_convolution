...
def train(args, model, device, train_loader, optimizer, optimizer_quant, epoch, log, pr1, pr2, ent=True):
    ...
    gamma,beta,alpha,clip = args.gamma, args.beta, args.alpha, args.clip
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if batch_idx % acc_batch == acc_batch_m1:
            ...
            acc_grad.acc(param)
        log.log('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} mse:{:.6f} ssim:{:.3} rate:{:.3}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss.item(),mse_loss.item(), 1-ssim_loss.item(), ent_loss.item()))

def test(args, model, device, test_loader,log, pr1, pr2):
    model.eval()
    sim_func = SSIM(11, 3).to(device)
    test_mse,test_ssim,test_ent = 0, 0, 0
    vd = args.valid_dim / 256. * .815
    for data in test_loader:
        with torch.no_grad():
            ...
            px = pr2(data)
            mse_loss = torch.mean((px-py)*(px-py)) 
            ssim_loss = sim_func(px,py)
            ent_loss = torch.sum(ent_vec)/torch.sum(mask).item()
            test_mse += mse_loss.item()  # sum up batch loss
            test_ent += ent_loss.item()
            test_ssim += ssim_loss.item()
    test_mse /= len(test_loader)
    test_ent /= len(test_loader)
    test_ssim /= len(test_loader)
    real_rt = vd * test_ent / 0.693 
    log.log('\nTest set: MSE loss: {:.6f}  ssim loss: {:.4f} Ent: {:.3f} rt: {:.3f}bpp'.format(test_mse, test_ssim, test_ent, real_rt))
    rt_loss = [test_mse-mse_tb(real_rt)] 
...
def Job(rank, world_size, args):
    ...
    prex = 'ent_{}_{}_{}_{}'.format('opt' if args.opt else 'normal', args.channels, args.valid_dim, args.npart) 
    ...
    if args.init:
        if os.path.exists('{}/save_models/{}_best_0.pt'.format(base_dir,prex)):
            of = '{}/save_models/{}_best_0.pt'.format(base_dir,prex)
        else:
            of = '{}/save_models/base_opt_192_{}_16_best_0.pt'.format(base_dir,args.valid_dim)
    ...
    log.log('valid dims:{} \t alpha:{}'.format(args.valid_dim,args.alpha))
    for epoch in range(1, args.epochs + 1):
        if not args.init and epoch % 4 == 1:
...
def main():
    ...
    parser.add_argument('--alpha', type=float, default=1, help='trade-off of rate loss')
