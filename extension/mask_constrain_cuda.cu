...
__global__ void conv_mask_v1_kernel(const int nthreads, scalar_t* const weight,
...
template <typename scalar_t>
__global__ void conv_mask_v2_kernel(const int nthreads, scalar_t* const weight,
    const int channel, const int sz, const int group_in, const int group_out){
    CUDA_KERNEL_LOOP(index, nthreads) {
        int tw = index % sz;
        int th = (index / sz) % sz;
	...
	AT_DISPATCH_FLOATING_TYPES(
		bottom_data.scalar_type(), "mask_constrain_forward_cuda", 
			([&] {
                    timer_->start();
                    if (constrain_==1){
                        conv_mask_v1_kernel<<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_>>>
                        (count, bottom_data.data_ptr<scalar_t>(), channel_, width_, group_in_, group_out_);
                    }else if (constrain_==2){
                        conv_mask_v2_kernel<<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_>>>
                        (count, bottom_data.data_ptr<scalar_t>(), channel_, width_, group_in_, group_out_);
                    }
                    else if(constrain_==5){
