...
__inline__  void cast_hindex_multiple(const int npart, int * tidx, const int mod){
    for(int i = 0; i<npart; i++){
      tidx[i] = (tidx[i] + mod - 1) / mod * mod;
    }
    return ;
}

  int sphere_cal_npart_hw_v2(const int height, const int width, const int npart, float* weight, int * tidx, int * hinv){
    //printf("%d %d\n",height, npart);
    assert("height should be multipler of the number of parts"&&(height%npart==0));
    int heights_per_part = height / npart;
    for(int i=0; i<npart; i++){
        tidx[i] = heights_per_part * (i+1);
    }
    float total = 0;
    for(int i=0;i<npart;i++){
      total += weight[i];
    ...
    if(total<3*npart){
      ...
      for(int i=0; i<npart; i++){
        tidx[i+npart] = static_cast<int>(weight[i]/64*width+0.5);
      }
    }
    for(int i=0, j=0; i<npart; i++){
      for(int k=j; k < tidx[i]; k++){
    ...
    //for(int i=0;i<height*2;i++) printf("%d ", hinv[i]);
    //printf("\n");
    //cast_hindex_multiple(npart, tidx+npart, 8);
    return tidx[npart/2]-tidx[npart/2-1];
  }

  void sphere_cal_npart_hw_v3(const int height, const int width, const int npart, float* weight, int * tidx){
    //printf("%d %d\n",height, npart);
    assert("height should be multipler of the number of parts"&&(height%npart==0));
    int heights_per_part = height / npart;
    float total = 0;
    ...
    if(total>3*npart){
      for(int i=0; i<npart; i++){
        tidx[i] = static_cast<int>(weight[i]/64*width+0.5);
      }
      return ;
    }
    float pi = acos(-1.0);
    if(npart%2==0){
        for(int i =0;i<npart/2-1;i++)
            tidx[i] = static_cast<int>(weight[i]*width * cos(((heights_per_part * (i+1) - 0.5)/height - 0.5)*pi)+0.5);
        tidx[npart/2-1] = width;
        tidx[npart/2] = width;
        for(int i = npart/2+1;i<npart;i++)
            tidx[i] = static_cast<int>(weight[i]*width * cos(((heights_per_part * i + 0.5)/height - 0.5)*pi)+0.5);
    }else{
        for(int i =0;i<npart/2;i++)
            tidx[i] = static_cast<int>(weight[i]*width * cos(((heights_per_part * (i+1) - 0.5)/height - 0.5)*pi)+0.5);
        tidx[npart/2 ] = width;
    ...
    //cast_hindex_multiple(npart, tidx, 8);
  }
